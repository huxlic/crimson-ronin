...
export const Navbar = () => {
	...
	return (
		<>
			<header
				className={"fixed top-0 w-full left-0 right-0 box-border px-8 bg-linear-to-t from-transparent to-[#0D0F11] z-50"}>
				<nav className={"flex items-center justify-between w-full py-4"}>
					...
					<ul className={"flex items-center font-marcellus gap-6"}>
						{navLinks.map(({href, title, icon: Icon}: NavLinks) => (
							<li key={title}
							    className={"text-[#ffffff99] hover:text-wuxia text-[12px] transition-colors duration-300 gap-1"}>
								<NavLink
